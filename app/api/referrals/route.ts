...
export async function POST(request: Request) {
  try {
    ...
    const claimed = await transaction(async (client) => {
      const self = await client.query("SELECT referred_by, referral_code, wallet_verified_at FROM users WHERE id = $1 FOR UPDATE", [identity.userId]);
      ...
      if (self.rows[0].wallet_verified_at) {
